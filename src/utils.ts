...
export function doGitMergeAndRebase(
  msgToCommit: string,
  dir: vscode.Uri,
  func: (result: GitMergeOrRebase) => void,
) {
...
export async function gitPush(dir: vscode.Uri) {
  ...
  try {
    ...
    vscode.window.showInformationMessage(
      "😄 SaveMeBaby: Git push success [" + code! + "]",
    );
  } catch (err: any) {
    vscode.window.showErrorMessage(
      "😞 SaveMeBaby: Git push failed, error: [" + err.stderr + "]",
    );
  }
  process.chdir(currDir);
}
 ...
 */
export async function startGitCommit(
  ...
  if (!fs.existsSync(file.fsPath)) {
    ...
    commitStatus.error =
      "File " + file.fragment + "Doesn't exist on disk yet, can't be commited";
    func(commitStatus);
    return;
  }
  if (logMsg === undefined) {
  ...
  try {
    ...
    if (gitCode === undefined) {
      commitStatus.status = "Error";
      commitStatus.error =
        "Couldn't get last Log message, can't confirm commit";
      func(commitStatus);
      return;
    }
