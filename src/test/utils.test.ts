...
import {
  ...
  getOrCreateNoCommitDirWithGitIgnore,
  getOrCreateOneCommitDir,
...
suite("Testing Utils", () => {
  test("Test getting parent dir", () => {
    ...
    const testingfile = vscode.Uri.file(
      path.join(__dirname, "testingfile.txt"),
    );
  ...
  test("Test dir is Git false since imaginary", () => {
    const testingfile = vscode.Uri.file(
      path.join(__dirname, "/hehehehhe/asdsadl/testingfile.txt"),
    );
    let parentDir = utils.getParentDir(testingfile);
    const result = utils.dirIsGit(parentDir);
  ...
  test("Test if can Start Git Commit on none commited git dir succesfully", async () => {
    ...
    const testFile = vscode.Uri.file(
      path.join(noCommitDir!.fsPath, "Test.txt"),
    );
  ...
  test("Test if can start Git Commit on dir with already previous commits", async () => {
    ...
    //create file
    const testFile = vscode.Uri.file(
      path.join(oneCommitDir!.fsPath, "yolo.txt"),
    );
  ...
  test("Test if can create commit on file that is inside .gitginore, should not", async () => {
    const gitIgnoreContent = `hehe.txt
*.toIgnore
toIgnore/*`;
    const noCommitWithGitIgoreDir: vscode.Uri | undefined =
      getOrCreateNoCommitDirWithGitIgnore(gitIgnoreContent);
    assert.notEqual(noCommitWithGitIgoreDir, undefined);

    //test saving hehe.txt
    const heheFile = vscode.Uri.file(
      path.join(noCommitWithGitIgoreDir!.fsPath, "hehe.txt"),
    );
    ...
    const toIgnoreFile = vscode.Uri.file(
      path.join(noCommitWithGitIgoreDir!.fsPath, "hehe.toIgnore"),
    );
    ...
    //testing saving for *.toIgnore file
    const toIgnoreFile2 = vscode.Uri.file(
      path.join(noCommitWithGitIgoreDir!.fsPath, "toIgnore/", "hehe.yolo"),
    );
    ...
    const notIgnore = vscode.Uri.file(
      path.join(noCommitWithGitIgoreDir!.fsPath, "notIgnore/", "not.yolo"),
    );
  ...
  test("Compressing previous commits", async () => {
    ...
    const heheFile = vscode.Uri.file(
      path.join(noCommit!.fsPath, "hehe_lol.txt"),
    );
    ...
    const logMessages: Array<utils.LogInfo> = utils.dirGetAllLogMessages(
      noCommit!,
    );
    ...
    await utils.doGitMergeAndRebase(
      msgToCommit,
      noCommit!,
      (inResult: utils.GitMergeOrRebase) => {
        result = inResult;
      },
    );
    assert.notStrictEqual(result, undefined);
    assert.strictEqual(result!.commited, true); //don't check for push, it's only possible on a real repo with a remote
    const currentLogMessages: Array<utils.LogInfo> = utils.dirGetAllLogMessages(
      noCommit!,
    );
